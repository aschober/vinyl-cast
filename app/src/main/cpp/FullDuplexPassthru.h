...
public:
    void setSkipLocalPlayback(bool skipLocalPlayback) {
        LOGD("FullDuplexPassthru - setSkipLocalPlayback");
        mSkipLocalPlayback = skipLocalPlayback;
    }

    void setAudioDataCallback(JavaVM* javaVm, jobject audioDataCallbackInstance, jmethodID audioDataCallbackMethod) {
    ...
    onBothStreamsReady(const void *inputData, int numInputFrames, void *outputData,
        ...
        if (bytesFromInput > 0 && mJavaVm != NULL) {
            JNIEnv* env;
            //TODO: figure out if/ when to detach from current thread
